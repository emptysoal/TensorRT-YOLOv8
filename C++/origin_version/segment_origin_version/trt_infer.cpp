...
void process_mask(float* protoDevice, Dims32 protoOutDims, std::vector<Detection>& vDetections, int kInputH, int kInputW, cv::Mat& img){
    ...
    CHECK(cudaMalloc(&maskCoefDevice, protoC * sizeof(float)));
    // prepare 160x160 mask space on device
    float* maskDevice = nullptr;
    CHECK(cudaMalloc(&maskDevice, protoH * protoW * sizeof(float)));

    // prepare for crop mask
    float widthRatio = (float)protoW / (float)kInputW;  // 160 / 640 = 0.25
    float heightRatio = (float)protoH / (float)kInputH;  // 160 / 640 = 0.25
    float downsampledBbox[4];  // save downsampled bbox from 640 to 160
    float* downBboxDevice = nullptr;
    CHECK(cudaMalloc(&downBboxDevice, 4 * sizeof(float)));

    // prepare for cut mask
    ...
    CHECK(cudaMalloc(&cutMaskDevice, cutMaskHeight * cutMaskWidth * sizeof(float)));

    // prepare for resize mask
    float* scaledMaskDevice = nullptr;
    CHECK(cudaMalloc(&scaledMaskDevice, img.rows * img.cols * sizeof(float)));

    for (size_t i = 0; i < vDetections.size(); i++){
        // mask = mask_coef * proto
        CHECK(cudaMemcpy(maskCoefDevice, vDetections[i].mask, protoC * sizeof(float), cudaMemcpyHostToDevice));
        process_mask_cuda(protoDevice, maskCoefDevice, maskDevice, protoC, protoH, protoW);

        // down sample bbox from 640x640 to 160x160
        downsampledBbox[0] = vDetections[i].bbox[0] * widthRatio;
        downsampledBbox[1] = vDetections[i].bbox[1] * heightRatio;
        downsampledBbox[2] = vDetections[i].bbox[2] * widthRatio;
        downsampledBbox[3] = vDetections[i].bbox[3] * heightRatio;
        // set 0 where mask out of bbox
        CHECK(cudaMemcpy(downBboxDevice, downsampledBbox, 4 * sizeof(float), cudaMemcpyHostToDevice));
        crop_mask(maskDevice, downBboxDevice, protoH, protoW);

        // scale mask from 160x160 to original resolution
        // 1. cut mask
        cut_mask(maskDevice, cutMaskDevice, cutMaskTop, cutMaskLeft, cutMaskHeight, cutMaskWidth, protoW);
        // 2. bilinear resize mask
        resize(cutMaskDevice, cutMaskHeight, cutMaskWidth, scaledMaskDevice, img.rows, img.cols);

        float* scaledMask = new float[img.rows * img.cols];
        CHECK(cudaMemcpy(scaledMask, scaledMaskDevice, img.rows * img.cols * sizeof(float), cudaMemcpyDeviceToHost));
        vDetections[i].maskMatrix = scaledMask;
    }

    CHECK(cudaFree(maskCoefDevice));
    CHECK(cudaFree(maskDevice));
    CHECK(cudaFree(downBboxDevice));
