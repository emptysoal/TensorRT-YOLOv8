...
- 根据 `GitHub` 上 `YOLOv8` 的 `README` 中的链接下载关键点检测模型，如：`yolov8s-pose.pt`；
- 在本地 `YOLOv8`一级 `ultralytics` 目录下，新建 `weights` 目录，并且放入下载的`yolov8s-pose.pt`模型
...
- 运行 `python export_onnx.py` 后，会在 `weights` 目录下生成 `yolov8s-pose.onnx`
